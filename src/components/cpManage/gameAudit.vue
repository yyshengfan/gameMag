    ...
    <div v-loading="loading" class="overFlowStyle">
      <el-row :gutter="10">
        <el-form  :model="model" :rules="rules" ref="form"  label-width="120px" >
          ...
          <el-form-item label="版权信息" prop="status2">
          ...
          <el-form-item label="游戏素材" prop="status3">
          ...
          <el-form-item label="申请营销资源" prop="status4">
          ...
          <el-form-item label="计费申请" prop="status5">
            <el-radio v-model="model.status5" label="2">审核通过</el-radio>
            <el-radio v-model="model.status5" label="1">审核不通过</el-radio>
          </el-form-item>
          <el-form-item  prop="remark5" v-if="model.status5=='1'">
...
<script>
  ...
  var model = function(){
    return {
      auditStatus:'0',status1:'2',remark1:'',status2:'2',remark2:'',status3:'2',remark3:'',status4:'2',remark4:'',status5:'2',remark5:'',
    }
  }
  form.props= ['dialogAudit','gameId']
  form.data=function () {
    return {
      loading: false,
      model:model(),
      ...
      rules:{
        ...
        remark4:[{ required: true, message: '请输入审核不通过原因', trigger: 'blur' },
          { min: 1, max: 500, message: '长度在 500字以内', trigger: 'blur' }],
        status5:[{ required: true, message: '请选审核意见', trigger: 'change' }],
        remark5:[{ required: true, message: '请输入审核不通过原因', trigger: 'blur' },
          { min: 1, max: 500, message: '长度在 500字以内', trigger: 'blur' }],
      }
    }
  };
  ...
  form.watch={
    ...
    'model.status5':function (newVal,oldVal) {
      if(newVal=='2'){
        this.model.remark5=''
      }
    },
        ...
        this.$http['post']('/auditManage/gameInfoAudit', this.model)
          .then((response) => {
            ...
            this.$alert('审核成功！', '温馨提示', {
              ...
              var temp=parseInt(this.model.status1)+parseInt(this.model.status2)+parseInt(this.model.status3)+parseInt(this.model.status4)
              if(temp==8){
               this.status=8
              }else{
                this.status=9
              }
              this.closeIt(this.status)
              this.resetForm('form')
            });
          })
          .catch((response) => {
